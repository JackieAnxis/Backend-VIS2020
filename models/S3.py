...
from models.gk_weisfeiler_lehman import compare
...
class S3():
    ...
    def __exemplar_dbscan__(self):
        dbs = DBSCAN(eps = self._epsilon, min_samples = 2,
                    metric = 'cosine', metric_params = None, algorithm = 'auto', leaf_size = 30, p = None,
...
def search_similar_structures(settings):
    ids = []
    tmp_vectors = []
    with open(settings['embedding_path'], 'r') as f:
        i = 0
        for line in f:
            if i is not 0:
                fields = line.split(',')
                id = int(float(fields[0]))
                vector = np.fromiter((float(x) for x in fields[1:]),
                                    dtype=np.float)
                tmp_vectors.append(vector)
                ids.append(id)
            i += 1
    vectors = np.zeros(shape=(len(tmp_vectors), tmp_vectors[0].shape[0]))
    i = 0
    for id in ids:
        vectors[id] = tmp_vectors[i]
        i += 1
    
    G = nx.read_edgelist(settings['edgelist_path'], nodetype=int,
                     data=(('weight', float),))
    matrix = nx.to_numpy_matrix(G, nodelist=range(0, len(G.nodes)))
    search_nodes = list(map(int, settings['search_nodes'])) # [183, 544, 569, 428, 523]

    
    s = S3(matrix, vectors)
    if 'min_nodes_threshold' in settings:
        s.min_nodes_threshold(settings['min_nodes_threshold'])
    if 'max_nodes_threshold' in settings:
        s.max_nodes_threshold(settings['max_nodes_threshold'])
    if 'k' in settings:
        s.k(settings['k'])
    if 'similarity_threshold' in settings:
        s.similarity_threshold(settings['similarity_threshold'])
    if 'epsilon' in settings:
        s.epsilon(settings['epsilon'])
...
if __name__ == '__main__':
    dataset_name = 'bn-mouse-kasthuri'  # replaced by request params
    path_prefix = '../data/'
    settings = {
        'min_nodes_threshold': 3,
        'max_nodes_threshold': 1000,
        'k': 100,
        'similarity_threshold': 0,
        'embedding_path': path_prefix + dataset_name + '/embedding.csv',
        'edgelist_path': path_prefix + dataset_name + '/graph.edgelist',
        'search_nodes': [55,56,411,362,453,380,712,928,573]
    }
