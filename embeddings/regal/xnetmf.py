...
import pandas as pd
from embeddings.regal.config import *
...
def feature(nx_graph):
	adj_matrix = nx.adjacency_matrix(nx_graph).todense()

	graph = Graph(adj_matrix)
	rep_method = RepMethod(max_layer=2)  # Learn representations with xNetMF.  Can adjust parameters (e.g. as in REGAL)
	feature = get_features(graph, rep_method)
	return feature
...
if __name__ == "__main__":
	...
	pd.DataFrame(representations).to_csv(path + 'xnetmf.csv')
